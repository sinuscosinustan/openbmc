...
function get_boot_stage_string()
{
	...
	case $bootstage in
		...
		02)
			boot_stage_str="TF-A BL1 (Code=${ueficode})"
			;;
		...
		05)
			boot_stage_str="TF-A BL2 (Code=${ueficode})"
			;;

		06)
			boot_stage_str="TF-A BL31 (Code=${ueficode})"
			;;

		07)
			boot_stage_str="TF-A BL32 (Code=${ueficode})"
			;;
...
PRIORITY=2
SEVERITY=
REDFISH_MESSAGE_ID=OpenBMC.0.5.BIOSBoot
...
PRIORITY=2
SEVERITY=
REDFISH_MESSAGE_ID=OpenBMC.0.5.BIOSFirmwarePanicReason
...
do
	# Sleep 200ms
	sleep 0.2
	bg="$(cat /sys/bus/platform/devices/smpro-misc.2.auto/boot_progress)"
	if [ -z "${bg}" ];
	then
		cnt=$((cnt + 1))
		continue
